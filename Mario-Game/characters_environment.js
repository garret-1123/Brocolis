...
function startGame() {
GameStatus = "start"
document.getElementById("status").innerHTML = "Prepara, que agora, é a hora, do show das poderosas"
...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Jogue para jogar o jogo", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(GameStatus== "start" && gameConfig.status==="start" && noseX != "") {
    document.getElementById("status").innerHTML = "Jogue"
    world_start.play();
...
function die(character){
    ...
    if (character.liveNumber > 0 ) die.play()
}
